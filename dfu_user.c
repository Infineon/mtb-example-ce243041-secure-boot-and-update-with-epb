 ...
 *
 * Description      : This file implements all user-defined hook functions required
 *                    by the DFU middleware, including transport callbacks, NVM read/write
 *                    operations, and address validation for the shared secondary slot.
 *
...
static bool AddressValid(uint32_t address, cy_stc_dfu_params_t *params)
{
    ...
    ((CY_EXT_NVM1_BASE <= address) && (address < (CY_EXT_NVM1_BASE + CY_EXT_NVM1_SIZE)));
        ...
        #else
            /* Valid only if the address falls in the secondary slots */
            addrValid = ((CYMEM_CM33_0_S_secondary_slot_S_START <= address) && (address < (CYMEM_CM33_0_S_secondary_slot_S_START + CYMEM_CM33_0_S_secondary_slot_SIZE)));
            CY_UNUSED_PARAMETER(params);
        #endif /* CY_IP_M7CPUSS */
    #endif /* (CY_DFU_OPT_EXTERNAL_MEMORY != 0U) */
#endif /* CY_DFU_FLOW == CY_DFU_BASIC_FLOW */
...
CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 11.3','Casting uint8_t* to uint32_t* is safe as input address is always valid and aligned.');
                ...
                fstatus = Cy_Flash_WriteRow(address, (uint32_t*)params->dataBuffer);
                mtb_hal_system_critical_section_exit(int_status);
                if(fstatus != CY_RSLT_SUCCESS)
                {
                    status = CY_DFU_ERROR_DATA;
